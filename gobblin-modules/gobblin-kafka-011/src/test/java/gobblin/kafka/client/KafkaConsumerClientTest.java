...
public class KafkaConsumerClientTest {
  ...
  public void testConsume() throws Exception {
    ...
    Config testConfig = ConfigFactory.parseMap(ImmutableMap.of(
        ConfigurationKeys.KAFKA_BROKERS, "test:1234", 
        KafkaDeserializerExtractor.KAFKA_DESERIALIZER_TYPE, "STRING",
        ConfigurationKeys.JOB_ID_KEY, "testJob"));

    MockConsumer<String, String> consumer = new MockConsumer<>(OffsetResetStrategy.NONE);
    ...
    try (KafkaConsumerClient<String, String> kafkaClient =
        new KafkaConsumerClient<>(testConfig, Optional.of(consumer))) {

      // Consume from 0 offset
      Set<KafkaConsumerRecord> consumedRecords =
          Sets.newHashSet(kafkaClient.consume(new KafkaPartition.Builder().withId(0).withTopicName("test_topic")
              .build(), 0l, 100l));

      Set<Kafka011ConsumerRecord<String, String>> expected =
          ImmutableSet.<Kafka011ConsumerRecord<String, String>> of(new Kafka011ConsumerRecord<>(record0),
              new Kafka011ConsumerRecord<>(record1), new Kafka011ConsumerRecord<>(record2));
